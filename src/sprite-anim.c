...
static struct SpriteAnim sSpriteAnims[ANIM_COUNT];

static void PutAnim(struct SpriteAnim * anim, int x, int y);
bool ExecAnim(struct SpriteAnim * anim);
...
struct ProcCmd CONST_DATA ProcScr_SpriteAnimProc[] = {
    PROC_SET_END_CB(AnimProc_OnEnd),
...
struct SpriteAnim * FindSpriteAnim(u16 const * info)
{
    ...
    for (i = 0; i < ANIM_COUNT; ++i)
    {
        if (sSpriteAnims[i].info == info)
            return &sSpriteAnims[i];
    }
...
ProcPtr StartSpriteAnimProc(u16 const * info, int x, int y, int oam2, int animid, int layer)
{
    ...
    proc = Proc_Start(ProcScr_SpriteAnimProc, PROC_TREE_3);
...
{
    struct ProcSpriteAnim * aproc = proc;
...
{
    Proc_EndEach(ProcScr_SpriteAnimProc);
}

bool SpriteAnimProcExists(void)
{
    return Proc_Exists(ProcScr_SpriteAnimProc);
}
