    ...
    for (i = 0; i < 4; i++)
    {
        ...
        if (proc->width[i] > 1)
        {
            if (proc->height[i] > 1)
            {
                ...
                PutSpriteExt(
                    0xd,
                    proc->x[i] + (proc->width[i] - 1) * 8,
                    proc->y[i],
                    Sprite_8x8,
                    proc->chr + proc->oam2[i] + 4);
                PutSpriteExt(
                    0xd,
                    proc->x[i] | OAM1_HFLIP | OAM1_VFLIP,
                    proc->y[i] + (proc->height[i] - 1) * 8,
                    Sprite_8x8,
                    proc->chr + proc->oam2[i] + 4);
                PutSpriteExt(
                    0xd,
                    (proc->x[i] + (proc->width[i] - 1) * 8) | OAM1_VFLIP,
                    proc->y[i] + (proc->height[i] - 1) * 8,
                    Sprite_8x8,
                    proc->chr + proc->oam2[i] + 4);
                ...
                for (; j > 1; j -= 2)
                {
                    PutSpriteExt(0xd, xOam1, yOam0, Sprite_16x8, proc->chr + proc->oam2[i]);
                    xOam1 = xOam1 + 0x10;
                }

                for (; j > 0; j--)
                {
                    PutSpriteExt(0xd, xOam1, yOam0, Sprite_8x8, proc->chr + proc->oam2[i]);
                    xOam1 = xOam1 + 8;
                }
...
void DisplayExtendedSysHand(struct SysHandCursorProc * proc)
{
    ...
    *_dst = *src;
    ...
    EnablePalSync();
    PutSpriteExt(4, proc->x, proc->y + 8, Sprite_8x8,
        OAM2_PAL(proc->pal_bank) + proc->chr2 + proc->chr);

    for (i = 1; i < proc->shadow_len; i++)
        PutSpriteExt(4, proc->x + i * 8, proc->y + 8, Sprite_8x8,
            OAM2_PAL(proc->pal_bank) + proc->chr2 + proc->chr + 1);

    PutSpriteExt(4, proc->x + proc->shadow_len * 8, proc->y + 8, Sprite_8x8,
...
void BmBgfx_End(struct ProcBmfx * proc)
{
    ...
    {
        SetBgChrOffset(proc->bg, proc->vram_base);
        TmFill(GetBgTilemap(proc->bg), 0);
        EnableBgSync(1 << proc->bg);
    }
}
...
void StartBmBgfx(struct BmBgxConf * input, int bg, int x, int y, int vram_off, int size, int pal_bank, void * func, ProcPtr parent)
{
    ...
    proc->vram_base = GetBgChrOffset(bg);
