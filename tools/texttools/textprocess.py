#!/bin/python3
# this aims to rebuild msg_data.txt
...
def text_to_shiftjis_u16_array(text, control_chars, encoding_method):
    ...
    while pos < len(text):
        ...
        if match:
            ...
            if preceding_text:
                sjis_bytes = preceding_text.encode(encoding_method)
            ...
            if remaining_text:
                sjis_bytes = remaining_text.encode(encoding_method)
                u16_array.extend(struct.unpack(f'{len(sjis_bytes)//2}H', sjis_bytes))
            break
...
def main(args):
    ...
    with open(output_data, 'w', encoding='utf-8') as data_file:
        data_file.write('#include "gbafe.h"\n\n')
